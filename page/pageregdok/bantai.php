...
session_start();
	 ...
	 */
	$aColumns = array('MetaId','Klasifikasi','Nomor','Tgl_Surat','Uraian', 'RoleName','KodeReg');
...
while($aRow = mysqli_fetch_array($rResult)){
    ...
    $row[] .= ''.$aRow['Nomor']." / ".$aRow['Tgl_Surat'].'';
    $row[] .= ''.$aRow['Uraian'];
    $row[] .= ''.$aRow['RoleName'];
    $row[] .= '<div align="center"><button type="button" title="Ubah Satuan Naskah" onclick=getload("'.$aRow['MetaId'].'"); class="btn btn-info btn-sm"><i class="fa fa-edit"></i></button>
               <button type="button" title="Hapus Satuan Dokumen" onclick=deldata("'.$aRow['MetaId'].'"); class="btn btn-danger btn-sm"><i class="fa fa-bitbucket"></i></button></div>';
    $output['aaData'][] = $row;
