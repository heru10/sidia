<script src="bower_components/bootstrap-datepicker/js/bootstrap-datepicker.js"></script>
  ...
  <section class="content-header">
      <h3>
        Daftar Isi Detail Dokumen Aktif
      </h3>
      <ol class="breadcrumb">
        <li><a href="index2.html"><i class="fa fa-dashboard"></i> Home</a></li>
        <li class="active">Registrasi Isi Berkas Arsip Aktif</li>
      </ol>
  </section>
        ...
        <!--<div class="col-md-9">-->
          <div class="box">
            <div class="box-header with-border">
              <h4 class="box-title">Daftar Isi Detail Dokumen Aktif >> <?php echo $xBerkas['Nomor']; ?> - <?php echo $xBerkas['Uraian']; ?></h4>
            </div>
            <div class="box-body">
                 ...
                 <td>
                    <select class="form-control select4" name="berkas" id="berkas" style="width: 50%;">
                      <option value="">~ Pilih Berkas ~</option>
                       <?php
                           include_once "../../conf.php";
                           $sql = "SELECT klasifikasi.ClCode, berkas.Uraian, berkas.BerkasId, berkas.Nomor FROM berkas
                                   INNER JOIN klasifikasi ON berkas.ClId = klasifikasi.ClId where berkas.RoleId = '".$_SESSION['RoleId']."' and berkas.StatusDok = 'aktif' ";
                           $result = mysqli_query($connect, $sql);
                  ...
                  <td>
                     <select class="form-control select2" name="txt1" id="txt1" style="width: 50%;" required>
                      <option value="">~ Pilih Klasifikasi Dokumen ~</option>
                       <?php
                           include_once "../../conf.php";
                           $sql = "select ClId, concat(ClCode,' - ',ClName) as ClKlas, RetensiThn_Active from klasifikasi where ClCode != 'SK'";
                           $result = mysqli_query($connect, $sql);
                           while($row=mysqli_fetch_array($result)){
                             echo "<option value='".$row['ClId']."'>".$row['ClKlas']."</option>";
                             $ClR .= $row['RetensiThn_Active'].'|';
                           }
                        ?>
                    </select>
