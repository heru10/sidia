...
//   }
  ...
  $(function(e){
   $("#mymetadata").on('submit',(function(e) {
        e.preventDefault();
       var urls = "<?php echo 'page/pageregdetailvitaldalitas/pendukung/pendukung_proses.php'; ?>";
...
function getload_pl(){
  ...
  var ids = $("#ids").val();
    $.ajax({
        url: "page/pageregdetailvitaldalitas/pendukung/pendukung_proses.php", // Url to which the request is send
        type: "POST",
        ...
        success: function(data){
          ...
          for(var i=0;i<ArryFile;i++){
           ...
           filex = filex.split("/");
           $("#fpl").append("<tr><td width=2%>"+no+".</td><td>"+filex[3]+"</td><td width=5%><button type='button' onclick=delfiles('"+data.items[i].DetVFileId+"'); title='Hapus File' class='btn btn-danger btn-sm' id='btndelfile'><i class='fa fa-bitbucket' ></i></button></td></tr>");
           no++;
          }
          folder = filex[0]+"/"+filex[1]+"/"+filex[2];
          $("#id").val(data.items[0].DetailVitalId);
          $("#ids").val(data.items[0].RegVitalId);
          $("#jmlfile_pl").val(ArryFile);
          $("#path").val(folder);
          $("#task").val('edit');
    ...
    $.ajax({
        ...
        success: function(data){
          ...
          $("#txt5").val(data.items[0].Keterangan);
        }
    })
}
   ...
   var id = $("#ids").val();
    ...
    if(tanya){
     ...
     $.ajax({
        ...
        pageurl("page/pageregdetailvitaldalitas/regdetailvitaldalitas.php?ids="+dat[1]);
        }
     })
