...
function fileExists(url) {
  ...
  if(fileExists(files[i].name)){
   ...
   if($("#task").val()!='edit'){
    ...
    $("#x"+name).html('');
    for (var i = 0; i < files.length; i++){
      $("#x"+name).append("<tr><td width=2%>"+no+".</td><td>"+files[i].name+"</td><td>"+humanFileSize(files[i].size,true)+"</tr></tr>");
     ...
     for (var i = 0; i < files.length; i++){
      no++;
      totsize = totsize + parseInt(files[i].size);
      $("#x"+name).append("<tr class='bg-blue'><td width=2%>"+no+".</td><td>"+files[i].name+"</td><td>"+humanFileSize(files[i].size,true)+"</td><td><span class='label label-info pull-right'>"+kp+"</span></td></tr>");
     }
     //$("#x"+name).append("<tr><td colspan='3' align='right' ><b>Total File Upload</b></td><td><b>"+humanFileSize(totsize,true)+"</b></td></tr>");
   }

   if(totsize > 100000000){
      toastr.error("Upload dokumen terlalu besar, maximal 100 MB !!", "Kesalahan");
      $('#txtfsp').val('');
      $("#f"+name).html('');
      $("#x"+name).html('');
    }
  ...
  $(function(e){
   $("#mymetadata").on('submit',(function(e) {
        e.preventDefault();
       var urls = "<?php echo 'page/pageregdetailvitaldalitas/spesimen/spesimen_proses.php'; ?>";
...
function getload_spesimen(){
  var id = $("#id").val();
  var ids = $("#ids").val();
    $.ajax({
        url: "page/pageregdetailvitaldalitas/spesimen/spesimen_proses.php", // Url to which the request is send
        type: "POST",
        data: "task=loadfile&ids="+ids,
        async: true,
        ...
        success: function(data){
          ...
          for(var i=0;i<ArryFile;i++){
           filex = data.items[i].Filename;
           filex = filex.split("/");
           $("#fsp").append("<tr><td width=2%>"+no+".</td><td>"+filex[3]+"</td><td>"+humanFileSize(data.items[i].Size,true)+"</td><td width=5%><button type='button' onclick=delfiles('"+data.items[i].DetVFileId+"'); title='Hapus File' class='btn btn-danger btn-sm' id='btndelfile'><i class='fa fa-bitbucket' ></i></button></td></tr>");
           no++;
          }
          folder = filex[0]+"/"+filex[1]+"/"+filex[2];
          $("#id").val(data.items[0].DetailVitalId);
          $("#ids").val(data.items[0].RegVitalId);
          $("#jmlfile_sp").val(ArryFile);
          $("#path").val(folder);
          $("#task").val('edit');
