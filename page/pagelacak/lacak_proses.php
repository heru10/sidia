...
$idcode = str_replace(" ","", $_REQUEST['id']);
...
$sql_sainprof2 = "SELECT
detailvitalmetafile_proof.Filename,
detailvitalmetafile_proof.Size,
detailvitalmetafile_proof.Ket,
detailvitalmetafile_proof.KodeReg,
DATE_FORMAT(detailvitalmeta_proof.Tgl_Desain,'%Y') AS Tahun,
detailvitalmeta_proof.Perihal as Ur,
v_naskah_vital_proof.RoleName,
CASE detailvitalmetafile_proof.Ket
WHEN 'a-dasar' THEN 'Dasar Permintaan'
WHEN 'b-lampiran' THEN 'Lampiran Dasar Permintaan'
WHEN 'c-nspk' THEN 'Surat Perintah Kerja'
WHEN 'd-nota' THEN 'Surat Penyerahan'
WHEN 'e-preview' THEN 'Surat Penyerahan'
END as Keterangan
FROM
detailvitalmeta_proof
INNER JOIN detailvitalmetafile_proof ON detailvitalmetafile_proof.DetailProfId = detailvitalmeta_proof.DetailProfId
INNER JOIN v_naskah_vital_proof ON detailvitalmeta_proof.RegVitalId = v_naskah_vital_proof.RegVitalId
where v_naskah_vital_proof.Roleid = '".$_SESSION['RoleId']."' and  detailvitalmetafile_proof.KodeReg = '".$idcode."' ";
//echo $sql_sainprof2;
$res_sainprof2 = mysqli_query($connect, $sql_sainprof2);
if(mysqli_num_rows($res_sainprof2)!=0){
$obj = mysqli_fetch_array($res_sainprof2);
$arr[] = $obj['Tahun'];
$arr[] = $obj['Ur'];
$arr[] = $obj['RoleName'];
$arr[] = $obj['Keterangan'];
