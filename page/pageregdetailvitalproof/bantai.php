...
session_start();
	 ...
	 */
	$aColumns = array('DetailProfId','RegVitalId','Tgl_Nodin','Dasar','Perihal','KetPekerjaan','Lokasi','Target');
...
while($aRow = mysqli_fetch_array($rResult)){
    ...
    $row[] .= ''.date('d-m-Y', strtotime($aRow['Tgl_Nodin'])).'';
    $row[] .= ''.$aRow['Dasar'].'';
    $row[] .= ''.$aRow['Perihal'].'';
    $row[] .= ''.$aRow['KetPekerjaan'].'';

    $target1 = explode('#',$aRow['Target']);
    $tgr = "";
    if($target1[0]<>'0000-00-00'){
       $tgr = $target1[0];
       $row[] .= '<center>'.date('d-m-Y',strtotime($tgr)).'</center>';
    }else if($target1[1]<>'0000-00-00'){
       $tgr = $target1[1];
       $row[] .= '<center>'.date('d-m-Y',strtotime($tgr)).'</center>';
    }else
       $row[] .= '<center>00-00-0000</center>';
