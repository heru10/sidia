...
//	$aColumns = array( 'NId', 'NTipe', 'StatusReceive', 'Nomor', 'Hal', 'Tgl', 'InstansiPengirim' );
	...
	if ($sWhere=="") {
		$sQuery = "
			...
			FROM   $sTable
			Where RoleId = '".$_SESSION['RoleId']."' and ParrentBerkasId='".$_GET['id']."' and SubBerkas = 'sub'  order by TglReg desc
            $sOrder
			$sLimit
		";
	} else {
		$sQuery = "
			SELECT SQL_CALC_FOUND_ROWS ".str_replace(" , ", " ", implode(", ", $aColumns))."
			FROM   $sTable
			$sWhere and RoleId = '".$_SESSION['RoleId']."' and ParrentBerkasId='".$_GET['id']."' and SubBerkas = 'sub'  order by TglReg desc
			$sOrder
			$sLimit
			";
	}
