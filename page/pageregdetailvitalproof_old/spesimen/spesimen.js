...
function getload_spesimen(){
  ...
  var ids = $("#ids").val();
    $.ajax({
        url: "page/pageregdetailvitalproof/spesimen/spesimen_proses.php", // Url to which the request is send
        type: "POST",
        ...
        success: function(data){
          ...
          for(var i=0;i<ArryFile;i++){
           ...
           filex = filex.split("/");
           $("#fsp").append("<tr><td width=2%>"+no+".</td><td>"+filex[3]+"</td><td width=5%><button type='button' onclick=delfiles('"+data.items[i].DetVFileId+"'); title='Hapus File' class='btn btn-danger btn-sm' id='btndelfile'><i class='fa fa-bitbucket' ></i></button></td></tr>");
           no++;
          }
