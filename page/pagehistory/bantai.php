...
while($aRow = mysqli_fetch_array($rResult)){
    ...
    $row[] .= ''.$aRow['PeopleName'];
    $logdok = $aRow['LogDok'];
    $logdok1 = str_replace('../','',$aRow['LogDok']);
    $logdok1 = str_replace(' ','%20',$logdok1);
    $gdok = explode("/",$logdok);
    ...
    $row[] .= '<a href='.$logdok1.' target="_blank"><b>'.$filet.'</b></a>';
    $row[] .= '<div align="center">'.date_format(date_create($aRow['LogDate']),'d/m/Y H:i:s').'</div>';
    $output['aaData'][] = $row;
}
