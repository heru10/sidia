...
while($aRow = mysqli_fetch_array($rResult)){
    ...
    $row[] .= '<center><i class="text-yellow fa fa-folder fa-2x" onclick=detail_vital("'.$aRow['RegVitalId'].'") style="cursor: pointer"></i></center>';
    $row[] .= '<div align="center"><button type="button" title="Ubah Satuan Naskah" onclick=getload("'.$aRow['RegVitalId'].'"); class="btn btn-info btn-sm"><i class="fa fa-edit"></i></button>
               <button type="button" title="Hapus Satuan Dokumen" onclick=deldata("'.$aRow['RegVitalId'].'"); class="btn btn-danger btn-sm"><i class="fa fa-bitbucket"></i></button></div>';
    $output['aaData'][] = $row;
