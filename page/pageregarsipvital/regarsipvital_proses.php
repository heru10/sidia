<?php
 ...
 if($task=='new'){
   try {
    ...
    $sql .= "(RegVitalId, KodeReg, KatProdukId, JProdukId,  DetailProdukId,
              ClId, RoleId, Uraian, KodeId, Tahun, TPId, MediaId, KondisiId, Jumlah, SatuanId,
              Dept, Tgl_Reg, Operator) ";
    $sql .= "values( ";
    $sql .= " '". $bvitalid ."', ";
    $sql .= " '" . $_POST['codereg'] . "', ";
    $sql .= " '" . $data[1] . "', ";
 ...
 if($task=='del'){
  try{
    ...
    $sql = "SELECT detailvitalfile.RegVitalId FROM
            detailvital
            INNER JOIN detailvitalfile ON detailvitalfile.RegVitalId = detailvital.RegVitalId
            WHERE detailvitalfile.RegVitalId = '".$id."' ";
