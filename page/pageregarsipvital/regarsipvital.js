...
function load_txt1(idjp){
   var id = $("#txt1").val();
    $.ajax({
      url  : 'page/pageregarsipvital/regarsipvital_proses.php',
...
function reTable(){
  var table = $('#mytbl').DataTable({
      ...
      'ajax'        : 'page/pageregarsipvital/bantai.php',
      'oLanguage': {
				"sSearch": "Pencarian :",
		}
    });
...
function detail_vital(id){
  pageurl('page/pageregdetailvital/regdetailvital.php?ids='+id);
}

$(function(e){
   $("#myfrm").on('submit',(function(e) {
        e.preventDefault();
       var urls = "<?php echo 'page/pageregarsipvital/regarsipvital_proses.php'; ?>";
       $.ajax({
        ...
        success: function(data)   // A function to be called if request succeeds
        {
          ...
          //$("#close").click();
          pageurl("page/pageregarsipvital/regarsipvital.php");
        }
       });
   }));
})
    ...
    if(tanya){
     ...
     $.ajax({
        ...
        switch(data){
            ...
            case "error"  : toastr.error("Terjadi kesalahan database", "KESALAHAN");
                            break;
            case "found"  : toastr.warning("Ada Detai Berkas didalamnya!! ", "PERHATIAN");
                            return false;
                            break;
          }
          pageurl("page/pageregarsipvital/regarsipvital.php");
   ...
   function getload(id){
    $.ajax({
        url: "page/pageregarsipvital/regarsipvital_proses.php", // Url to which the request is send
        type: "POST",
...
//          $("#noreg").html("Nomor Reg. "+data.items.KodeReg);
...
function delfile(id){
  $.ajax({
        url: "page/pageregvital/regvital_proses.php", // Url to which the request is send
        type: "POST",
