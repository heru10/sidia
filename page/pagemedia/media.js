...
$(function () {
    ...
    $("#forminput").hide();
    $("#formtable").show();
    $("#lblstatus").html("Status : Tambah Media");
    $("#task").val("new");
    $("#id").val("0");
  });
...
function reTable(){
  var table = $('#mytbl').DataTable({
      ...
      'ajax'        : 'page/pagemedia/bantai.php'
    });
}
...
$(function(e){
   $("#myfrm").on('submit',(function(e) {
       ...
       $.ajax({
        url: "page/pagemedia/media_proses.php", // Url to which the request is send
        ...
        success: function(data)   // A function to be called if request succeeds
        {
          ...
          //$("#close").click();
          pageurl("page/pagemedia/media.php");
...
//   })
   ...
   function deldata(id){
    var tanya = confirm("Apakah Nama Media ini mau dihapus ?");
    if(tanya){
     $.ajax({
        url: "page/pagemedia/media_proses.php", // Url to which the request is send
        type: "POST",
        ...
        switch(data){
            ...
            case "error"  : toastr.error("Terjadi kesalahan database", "KESALAHAN");
                            break;
            case "found"  : toastr.warning("Medata media dokumen tidak bisa dihapus, karena sudah digunakan ", "PERHATIAN");
                            break;
          }
        pageurl("page/pagemedia/media.php");
        }
   ...
   function getload(id){
    ...
    $.ajax({
        ...
        success: function(data){
          $("#forminput").show();
          $("#formtable").hide();
          $("#lblstatus").html("Status : Ubah Media");
          $("#txt1").val(data.items.MediaName);
          $("#task").val('edit');
          $("#id").val(data.items.MediaId);
          $("#txt1").focus();
        }
     });
