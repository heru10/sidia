...
function format ( d ) {
    ...
    var shtml = $.ajax({
    type: "POST",
    url: "page/pagedaftardokberkas_v1/detail.php?id="+d,
...
function reTable(){
  var table = $('#mytbl').DataTable({
      ...
      'ajax'        : 'page/pagedaftardokberkas_v1/bantai.php',
      "columns": [
            {
...
function reTable2(){
  var table = $('#mytbl2').DataTable({
      ...
      "pageLength"  : 10,
      'PaginationType': "full_numbers",
      'ajax'        : 'page/pagedaftardokberkas_v1/bantai1.php?id='+$("#id").val(),
      "columns": [
            {
                "className":      'details-control',
                "orderable":      true,
   ...
   function closed2(){
    var id = $("#idx").val();
    pageurl("page/pagedaftardokberkas_v1/daftardoksubberkas.php?id="+id+"&req=sub");
   }

   function deldata(id){
    var tanya = confirm("Apakah metadata dokumen ini mau dihapus ?");
    if(tanya){
     var urls = "<?php echo 'page/pagedaftardokberkas_v1/daftardokberkas_proses.php'; ?>";
     $.ajax({
   ...
   function getload(id){
    ...
    $.ajax({
        url: "page/pagedaftardokberkas_v1/daftardokberkas_proses.php", // Url to which the request is send
        type: "POST",
        ...
        success: function(data){
          ...
          $("#lblcoddaftar").html("Nomor daftar. "+data.items.Kodedaftar);
          $("#codedaftar").html(data.items.Kodedaftar);
...
function detail_berkas(id,idx){
  //alert(id+idx);
  pageurl("page/pagedaftararsip_v1/daftararsip.php?id="+id+"&idx="+idx);
}
