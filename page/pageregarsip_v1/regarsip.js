...
$(function () {
    ...
    $("#forminput").hide();
    $("#formtable").show();
    $("#lblstatus").html("Status : Tambah Isi Detail Dokumen Vital");
    $("#task").val("new");
    //$("#id").val("0");
  });
...
function view_file(id){
  var ids = $("#idx").val();
  var idr = $("#id").val();
  pageurl("page/pageregarsip_v1/test_img.php?pfile="+id+"&ids="+ids+"&idr="+idr);
}
...
function reTable(){
  ...
  var table = $('#mytbl').DataTable({
      ...
      'ajax'        : 'page/pageregarsip_v1/bantai.php?idx='+id,
      'oLanguage': {
				"sSearch": "Pencarian :",
		},
      "columns": [
...
$(function(e){
   ...
   $("#myfrm").on('submit',(function(e) {
        e.preventDefault();
       var urls = "<?php echo 'page/pageregarsip_v1/regarsip_proses.php'; ?>";
       $.ajax({
        ...
        success: function(data)   // A function to be called if request succeeds
        {
          ...
          //$("#close").click();
          pageurl("page/pageregarsip_v1/regarsip.php?id="+id+"&idx="+idx+"&idr="+idr);
   ...
   function backpage(){
     ...
     pageurl("page/pageregdokberkas_v1/regdoksubberkas.php?id="+idx+"&req=sub");
   }

   function deldata(idr){
    id = $("#id").val();
    idx = $("#idx").val();
   ...
   function getload(id){
    ...
    //alert(id);
    $.ajax({
        url: "page/pageregarsip_v1/regarsip_proses.php", // Url to which the request is send
        type: "POST",
        ...
        success: function(data){
          ...
          $("#task").val('edit');
          $("#idr").val(data.items['meta'][0].DokumenId);
          $("#txt1").focus();
          $("#jmlfile").val('0');
