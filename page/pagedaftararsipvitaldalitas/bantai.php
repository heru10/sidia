...
session_start();
	 ...
	 */
	$aColumns = array('RegVitalId','KatProdukName','JProdukName','DetailProdukName','Tahun','Uraian');
...
//	$aColumns = array( 'NId', 'NTipe', 'StatusReceive', 'Nomor', 'Hal', 'Tgl', 'InstansiPengirim' );
	...
	if ($sWhere=="") {
		$sQuery = "
			...
			FROM   $sTable
			Where RoleId like '".$_SESSION['RoleParentId']."%'
            $sOrder
			$sLimit
		";
	} else {
		$sQuery = "
			SELECT SQL_CALC_FOUND_ROWS ".str_replace(" , ", " ", implode(", ", $aColumns))."
			FROM   $sTable
			$sWhere and RoleId like '".$_SESSION['RoleParentId']."%'
			$sOrder
			$sLimit
			";
	}
	...
	$sQuery = "
		...
		FROM   $sTable Where RoleId like '".$_SESSION['RoleParentId']."%'
	";
...
while($aRow = mysqli_fetch_array($rResult)){
    ...
    $sql = "select * from detailvital where RegVitalId = '".$aRow['RegVitalId']."' ";
    $result = mysqli_query($connect,$sql);
    $jmlitem = mysqli_num_rows($result);
