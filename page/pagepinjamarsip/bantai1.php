...
session_start();
	 ...
	 */
	$aColumns = array('BerkasId', 'Ur', 'KurunWaktu');
...
while($aRow = mysqli_fetch_array($rResult)){
    ...
    $row[] .= '<center><input type="checkbox" name="chk[]" id="chks" value="'.$aRow['BerkasId'].'"></center>';
    $sql = "select ArsipPinjam from v_peminjaman_fisik where Keterangan = '".$_GET['ka']."' and StatusPinjam = '1'   ";
    $result = mysqli_query($connect, $sql);
    if(mysqli_num_rows($result)){
    while($Rdata=mysqli_fetch_array($result)){
      $Pdata = explode("|",$Rdata['ArsipPinjam']);
      foreach($Pdata as $Fdata){
        if($Fdata!=$aRow['BerkasId']){
          $output['aaData'][] = $row;
          $no++;
       }
      }
  ...
  while($aRow = mysqli_fetch_array($rResult)){
    ...
    $row[] .= '<center><input type="checkbox" '.$chklist.' name="chk[]" id="chks" value="'.$aRow['BerkasId'].'"></center>';
    $output['aaData'][] = $row;
    $no++;
  }
}
