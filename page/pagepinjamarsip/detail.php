<?php
 ...
 $sql = "select PeminjamanId, Filename, Keterangan, ArsipPinjam, Item, StatusPinjam, UnitKerja from v_peminjaman_fisik where PeminjamanId = '".$_GET['id']."' and RoleId_Reg = '".$_SESSION['RoleId']."' ";
...
?>
 ...
 <div class="box box-primary">
     ...
     <div class="box-body body-success">
      <div style="display: <?php echo $btnback; ?>"><button style="display: none" type="button" title="Registrasi Pengembalian Dokumen" onclick="getkembali()" id="btn_modal" data-toggle="modal" class="btn btn-success btn-sm">Registrasi Pengembalian Dokumen</button><br /><br /></div>
      <input type="hidden" id="idp" value="<?php echo $rows['PeminjamanId'] ?>" />
      <input type="hidden" id="uk"  value="<?php echo $rows['UnitKerja'] ?>" />
      <p>Surat Permohonan : <a target="_blank" href="<?php echo $rows['Filename']; ?>"><b><?php echo $files[3]; ?></b></a></p>
      ...
      <tr class="bg-yellow">
       ...
       foreach($bid as $it){
         switch ($rows['Keterangan']){
          ...
          //echo "<td><center><a target='_blank' href='".$rows['Filename']."' class='btn btn-info'><i class='fa fa-download'></i></a></center></td>";
