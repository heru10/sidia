<?php
 ...
 $sql = "select * from v_naskah where MetaId = '".$_GET['id']."' ";
 ...
 $html .= '<tr><td style="width: 15%">Tanggal Dokumen</td><td style="width: 1%">:</td><td style="width: 86%">'.$rows['Tgl_Surat'].'</td></tr>';
 $html .= '<tr><td>Jenis Dokumen</td><td>:</td><td>'.$rows['JenisNaskah'].'</td></tr>';
 $html .= '<tr><td valign="top">Uraian Dokumen</td><td valign="top">:</td><td>'.$rows['Uraian'].'</td></tr>';
 ...
 $html .= '<tr><td>Vital/Tidak Vital</td><td>:</td><td>'.$vital.'</td></tr>';
 ...
 $html .= '<select id="fileselect" style="width: 40%; display:'.$Fview.'" class="form-control input-sm" onchange=loadfile("'.$rows['CodeRole'].'");>';
           if($JFile != 0){
