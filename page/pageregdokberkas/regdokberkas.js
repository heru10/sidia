...
$(function () {
    ...
    $("#forminput").hide();
    $("#formtable").show();
    $("#lblstatus").html("Status : Tambah Berkas Aktif");
    $("#task").val("new");
    $("#id").val("0");
  });
...
function format ( d ) {
    ...
    var shtml = $.ajax({
    type: "POST",
    url: "page/pageregdokberkas/detail.php?id="+d,
...
function reTable(){
  var table = $('#mytbl').DataTable({
      ...
      'ajax'        : 'page/pageregdokberkas/bantai.php',
      "columns": [
            {
...
$(function(e){
   $("#myfrm").on('submit',(function(e) {
        e.preventDefault();
       var urls = "<?php echo 'page/pageregdokberkas/regdokberkas_proses.php'; ?>";
       $.ajax({
   ...
   function getload(id){
    ...
    $.ajax({
        url: "page/pageregdokberkas/regdokberkas_proses.php", // Url to which the request is send
        type: "POST",
...
function detail_berkas(id){
  //alert(id);
  pageurl("page/pageregarsip/regarsip.php?id="+id);
}
