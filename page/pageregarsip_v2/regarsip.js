...
$(function () {
    ...
    $("#forminput").hide();
    $("#formtable").show();
    $("#lblstatus").html("Status : Tambah Berkas Dokumen");
    $("#task").val("new");
    $("#id").val("0");
  });
...
function view_file(id){
  ...
  var idsb = $("#idsb").val();
  var req = $("#request").val();
  pageurl("page/pageregarsip_v2/test_img.php?pfile="+id+"&idb="+idb+"&idsb="+idsb+"&req="+req);
}
...
function reTable(){
  ...
  var table = $('#mytbl').DataTable({
      ...
      'ajax'        : 'page/pageregarsip_v2/bantai.php?idb='+idb,
      'oLanguage': {
				"sSearch": "Pencarian :",
		},
      "columns": [
...
$(function(e){
   $("#myfrm").on('submit',(function(e) {
        e.preventDefault();
       var urls = "<?php echo 'page/pageregarsip_v2/regarsip_proses.php'; ?>";
       $.ajax({
   ...
   function closed(){
    ...
    var req = $("#request").val();
    pageurl("page/pageregarsip_v2/regarsip.php?idb="+idb+"&idsb="+idsb+"&req="+req);
    //pageurl("page/pageregarsip_v2/regarsip.php?idx="+idx+"&idr="+idr);
   }

   function backpage(){
    if($("#request").val()=='subberkas'){
     var idsb = $("#idb").val();
     pageurl("page/pageregdokberkas_v2/regdoksubberkas.php?idberkas="+idsb);
    }else
    ...
    if(tanya){
     ...
     $.ajax({
        ...
        switch(data){
          ...
          var idb = $("#idb").val();
          var idsb = $("#idsb").val();
          pageurl("page/pageregarsip_v2/regarsip.php?idb="+idb+"&idsb="+idsb);
        }
     })
   ...
   function getload(id){
    ...
    //alert(id);
    $.ajax({
        url: "page/pageregarsip_v2/regarsip_proses.php", // Url to which the request is send
        type: "POST",
        ...
        success: function(data){
          ...
          $("#task").val('edit');
          $("#idx").val(data.items['meta'][0].DokumenId);
          $("#idberkas").val(data.items['meta'][0].BerkasId);
          $("#txt1").focus();
          $("#jmlfile").val('0');
          ...
          for(var i=0;i<carryIMG;i++){
            ...
            $("#filebox").show();
            $("#olfiles").append("<tr class='bg-warning'><td align='right'>"+no+".</td><td width='30%'>"+filer[3]+"</td><td align=right width='30%'>"+humanFileSize(data.items['img'][i].FileSize,true)+"</td><td><button type='button' onclick=delfiles('"+data.items['img'][i].FilesId+"'); title='Hapus File' class='btn btn-danger btn-sm' id='btndelfile'><i class='fa fa-bitbucket' ></i></button></td></tr>");
            no++;
          }
          $("#olfiles").append("<tr class='bg-info'><td colspan=2 align='right'><b>Total File :</b> </td><td align='right'><strong>"+humanFileSize(totsize,true)+"</strong></td><td>&nbsp;</td></tr>");
        }
     });
   }
...
function getupload(){
   ...
   var extfile = ['jpg','jpeg','pdf','JPG','JPEG'];
   var no = $("#jmlfile").val();
   var totsize = 0;
   ...
   if($("#task").val()=='new'){
    ...
    if($("#task").val()=='new'){
       ...
       $("#filebox").show();
       $("#olfiles").append("<tr class='bg-warning'><td align='right'>"+no+".</td><td width='30%'>"+files[i].name+"</td><td width='40%'>"+humanFileSize(files[i].size,true)+"</td></tr>");
       no++;
       ...
       $("#filebox").show();
       totsize = totsize + (files[i].size);
       $("#oxfiles").append("<tr class='bg-warning'><td align='right'>"+no+".</td><td width='30%'>"+files[i].name+"</td><td width='40%'>"+humanFileSize(files[i].size,true)+"</td><td><div class='label label-info label-xs'>Baru</div></td></tr>");
