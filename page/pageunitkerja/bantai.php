...
session_start();
	 ...
	 */
	$aColumns = array( 'RoleId', 'RoleName', 'Deskripsi', 'GroupName', 'KodeUnit' );
...
//	$aColumns = array( 'NId', 'NTipe', 'StatusReceive', 'Nomor', 'Hal', 'Tgl', 'InstansiPengirim' );
	...
	if ($sWhere=="") {
		...
		$sQuery = "
			SELECT SQL_CALC_FOUND_ROWS ".str_replace(" , ", " ", implode(", ", $aColumns))."
			FROM   $sTable
			$sWhere and RoleId not in ('Root','UK')
			Order by RoleId asc
			$sLimit
			";
	}
...
foreach($rResult as $aRow){
    ...
    $row[] .= '<div align="right">'.$aRow['KodeUnit'].'</div>';
    $row[] .= ''.$aRow['RoleName'].'';
    $row[] .= ''.$aRow['GroupName'].'';

    if($aRow['RoleId']!='UK'){
    $row[] .= '<div align="center"><button type="button" title="Ubah Unit Kerja" onclick=getload("'.$aRow['RoleId'].'"); class="btn btn-info btn-sm"><i class="fa fa-edit"></i></button>
               <button type="button" title="Hapus Unit Kerja" onclick=deldata("'.$aRow['RoleId'].'"); class="btn btn-danger btn-sm"><i class="fa fa-bitbucket"></i></button></div>';
    } else {
