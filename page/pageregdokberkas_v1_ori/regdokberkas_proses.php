  ...
  if($task=='loadberkas'){
   ...
   // die($sql);
	...
	if ($req!='sub')
	{
		...
		$sub_berkas = array();
		$sub[1] = "IJAZAH";
		$sub[2] = "BERKAS LAMARAN PEKERJAAN";
		$sub[3] = "HUKUMAN";
		$sub[4] = "DATA KELUARGA";
		$sub[5] = "KARTU IDENTITAS";
		$sub[6] = "PERJANJIAN KERJA";
		$sub[7] = "PROMOSI MUTASI";
		$sub[8] = "SERTIFIKAT";
		$sub[9] = "SK PENGANGKATAN";
		$sub[10] = "GAJI";
				
		$sql = "select BerkasId, ClId from berkas_v1 where RoleId = '".$_SESSION['RoleId']."' ";
		$result = mysqli_query($connect, $sql);
		$row = mysqli_fetch_array($result);
